...
// TODO: Here we are only storing the full name of  the user in the booking list not the email and number of tickets
// TODO : we can store this in the following format
// * first_name : Steve
// * last_name : Rogers
...
var bookings = make([]map[string]string, 1)  // ? Slice to store booking details
...
func book_tickets(user_tickets uint, first_name string, last_name string, email string) []map[string]string {
	...
	// * Now in map we cannot mix data types of values so we need to convert tickets value which is uint to string
	user_data["tickets_booked"] = strconv.FormatUint(uint64(user_tickets), 10)
